import { fetchLatestRemotePrice } from "./remoteMarket.ts";
...
export async function checkPriceAlertsInBackground(db: D1Database): Promise<{ checked: number; triggered: number }> {
  ...
  for (const [code, items] of byCode) {
    try {
      const price = await fetchLatestRemotePrice(code);
      const statements = items.map((alert) => {
      ...
      // One provider failure must not prevent other symbols from being checked.
    }
  }
