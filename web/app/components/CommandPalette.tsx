    ...
    <div className="command-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section className="command-palette" role="dialog" aria-modal="true" aria-label="TrendSight 命令中心" onKeyDown={(event) => {
        ...
        <div className="command-search">
          ...
          />
          <kbd>ESC</kbd>
          <button className="command-close" type="button" onClick={close} aria-label="关闭命令中心">完成</button>
        </div>
        <div className="command-results" id="command-results" role="listbox" aria-label="可用命令">
          {filtered.map((item, index) => <button id={`command-${item.id}`} key={item.id} type="button" role="option" aria-selected={active === index} className={active === index ? "active" : ""} onMouseEnter={() => setActive(index)} onClick={() => execute(item)}><span><strong>{item.label}</strong><small>{item.description}</small></span>{item.shortcut ? <kbd>{item.shortcut}</kbd> : null}</button>)}
