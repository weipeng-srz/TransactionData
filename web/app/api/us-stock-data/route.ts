...
export async function POST(request: Request) {
  let input: unknown;
  try {
    input = await safeJson(request);
    normalizeUSMarketRequest(input);
  } catch (reason) {
    return Response.json({ error: reason instanceof Error ? reason.message : "美股行情请求无效" }, { status: 400, headers: { "Cache-Control": "no-store" } });
  }
  ...
  try {
    ...
    return Response.json({ error: reason instanceof Error ? reason.message : "获取美股行情失败" }, { status: 502, headers: { "Cache-Control": "no-store" } });
  }
}
