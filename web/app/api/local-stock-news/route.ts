...
export async function POST(request: Request) {
  ...
  try {
    ...
    ({ code, limit } = normalizeRemoteNewsRequest(input));
  } catch (reason) {
    return Response.json({ error: reason instanceof Error ? reason.message : "新闻请求无效" }, { status: 400, headers: { "Cache-Control": "no-store" } });
  }
  ...
  try {
    ...
    return Response.json({ error: reason instanceof Error ? reason.message : "获取新闻数据失败" }, { status: 502, headers: { "Cache-Control": "no-store" } });
  }
}
