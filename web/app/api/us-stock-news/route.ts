...
export async function POST(request: Request) {
  try {
    const { code, limit } = normalizeUSNewsRequest(await safeJson(request));
    ...
    return Response.json({ error: reason instanceof Error ? reason.message : "获取美股新闻失败" }, { status: 400, headers: { "Cache-Control": "no-store" } });
  }
}
